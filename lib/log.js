...
function makeLevels(cfg) {
    ...
    return _.zipObject(cfg.levels, _.map(cfg.levels, function(level) {return idx--;}));
}
...
function format(format, args, json, colorizefn) {
    ...
    var props = _.extend({level: args[0],message: args[1]}, (args.length > 2 && _.isObject(args[2]))
    	? args[2]
        : _.zipObject(_.pull(_.clone(format), 'level','message'), _.tail(args,2))
	);
	...
	if (json) {
    	return JSON.stringify(_.zipObject(format, formatted));
    }
